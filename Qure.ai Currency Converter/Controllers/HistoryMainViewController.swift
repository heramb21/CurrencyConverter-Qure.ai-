...
class HistoryMainViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func btnDisplayRatesAction(_ sender: Any) {
        ...
        if tblCurrencies.indexPathForSelectedRow != nil {
            self.performSegue(withIdentifier: "showHistory", sender: nil)
        } else {
            self.showAlertMessage(titleStr: "Error", messageStr: "Please select currency")
        }
    ...
    private func setupDatePicker() {
        ...
        datePickerRate.semanticContentAttribute = .forceLeftToRight
        datePickerRate.subviews.first?.semanticContentAttribute = .forceLeftToRight
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showHistory" {
            if let vc = segue.destination as? HistoryDetailsViewController {
